...
func TestCPUTopologyFormModelInterface(t *testing.T) {
	...
	// First position should be a core toggle (if scan succeeded)
	if formModel.scanErr == nil && len(formModel.hostTopo.Dies) > 0 {
		firstPos := positions[0]
		if firstPos.Kind != form.FocusToggle || firstPos.Data == nil {
			t.Errorf("Expected first position to be a core toggle, got Kind=%v Key=%s", firstPos.Kind, firstPos.Key)
		}
	}
	...
	// Position count = N (core toggles) + 1 (save)
	if formModel.scanErr == nil && len(formModel.hostTopo.Dies) > 0 {
		expectedCoreToggles := formModel.hostTopo.TotalCores
		expectedTotal := expectedCoreToggles + 1
		if len(positions) != expectedTotal {
			t.Errorf("Expected %d positions (%d cores + 1 save), got %d", expectedTotal, expectedCoreToggles, len(positions))
		}
	}
}
