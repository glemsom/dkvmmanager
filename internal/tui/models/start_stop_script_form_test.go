...
func TestCustomScriptFormInit(t *testing.T) {
	...
	if fm.focusIndex != 0 {
		t.Errorf("Initial focusIndex = %d, want 0", fm.focusIndex)
	}
...
func TestCustomScriptFormToggleBuiltin(t *testing.T) {
	...
	if len(fm.positions) != 7 {
		t.Errorf("Expected 7 positions in custom mode initially, got %d", len(fm.positions))
	}
	...
	if len(fm.positions) != 3 {
		t.Errorf("Expected 3 positions in builtin mode, got %d", len(fm.positions))
	}
	...
	if len(fm.positions) != 7 {
		t.Errorf("Expected 7 positions in custom mode, got %d", len(fm.positions))
	}
}
...
func TestCustomScriptFormNavigation(t *testing.T) {
	...
	if fm.currentPos().Key != "toggle" {
		t.Fatalf("Expected initial key toggle, got %s", fm.currentPos().Key)
	}
...
func TestCustomScriptFormRebuildPositions(t *testing.T) {
	...
	fm.rebuildPositions()
	if len(fm.positions) != 3 {
		t.Errorf("Builtin mode: expected 3 positions, got %d", len(fm.positions))
	}
	...
	fm.rebuildPositions()
	if len(fm.positions) != 7 {
		t.Errorf("Custom mode: expected 7 positions, got %d", len(fm.positions))
	}
	...
	fm.rebuildPositions()
	if fm.focusIndex != 0 {
		t.Errorf("Expected focusIndex clamped to 0, got %d", fm.focusIndex)
	}
}
...
func TestCustomScriptFormEffectiveCursor(t *testing.T) {
	...
	val := fm.effectiveCursor("test", "hello")
	if val != 5 {
		t.Errorf("Expected cursor at end (5), got %d", val)
	}

	// Set offset
	fm.setCursorOffset("test", 2)
	val = fm.effectiveCursor("test", "hello")
	if val != 2 {
...
func TestCustomScriptFormViewContent(t *testing.T) {
	...
	// View() now calls syncViewport, so renderedLines should be populated
	_ = fm.View()

	if len(fm.renderedLines) == 0 {
		t.Error("Expected rendered lines after SetSize and View()")
	}
}
...
func TestEnterOnStopBrowseCreatesFileBrowser(t *testing.T) {
	...
	pos2 := fm.positions[fm.focusIndex]
	if pos2.Kind != form.FocusButton || pos2.Key != "stop_browse" {
		t.Fatalf("Expected focus on stop_browse, got Kind=%d Key=%s", pos2.Kind, pos2.Key)
	}
...
func TestFileSelectedStartPath(t *testing.T) {
	...
	fm.focusIndex = 2 // start_browse

	// Simulate file selection
	msg := FileSelectedMsg{Path: "/path/to/start-script.sh", Canceled: false}
	fm.handleFileSelected(msg)

	if fm.fileBrowser != nil {
		t.Error("Expected fileBrowser to be cleared after selection")
	}
	if fm.config.StartScript != "/path/to/start-script.sh" {
		t.Errorf("Expected StartScript='/path/to/start-script.sh', got '%s'", fm.config.StartScript)
	}
}
...
func TestFileSelectedStopPath(t *testing.T) {
	...
	if fm.config.StopScript != "/path/to/stop-script.sh" {
		t.Errorf("Expected StopScript='/path/to/stop-script.sh', got '%s'", fm.config.StopScript)
	}
}
...
func TestFileSelectedCanceled(t *testing.T) {
	...
	// Pre-set a path
	fm.config.StartScript = "/existing/script.sh"
	fm.fileBrowser = NewFileBrowserModel(FileTypeAll)
	fm.focusIndex = 2 // start_browse
	...
	if fm.config.StartScript != "/existing/script.sh" {
		t.Errorf("Expected StartScript unchanged, got '%s'", fm.config.StartScript)
	}
}
