...
func (r *VMRunner) buildQEMUArgs(vmDataDir string) []string {
	...
	if r.runCfg.CPUTopology.Enabled && len(r.runCfg.CPUTopology.SelectedCPUs) > 0 {
		...
		if r.runCfg.CPUTopology.UseHostTopology && len(hostTopo.Dies) > 0 && hostTopo.TotalCPUs > 0 {
			...
			// The -device declarations are generated by GenerateAsymmetricCPUDevices.
			...
			// Generate explicit -device declarations for asymmetric CPU topology.
			deviceArgs, _, err := GenerateAsymmetricCPUDevices(r.runCfg.CPUTopology.SelectedCPUs, hostTopo)
			if err != nil {
				// Runtime resilience: log warning and fall back to flat topology
				log.Printf("[WARNING] VM %s: asymmetric CPU topology generation failed: %v; falling back to flat topology",
					r.vm.Name, err)
